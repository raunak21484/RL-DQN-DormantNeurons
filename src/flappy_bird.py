...
class FlappyBird(object):
    ...
    def next_frame(self, action):
        pump()
        reward = random.uniform(0,random.uniform(0,0.2))
        terminal = False
        # Check input action
        if action == 1:
        ...
        for pipe in self.pipes:
            ...
            if pipe_center_x < bird_center_x < pipe_center_x + 5:
                self.score += 1
                print("CROSSED!!")
                reward =  random.uniform(random.uniform(2, 6), random.uniform(8, 17))
                # reward = 3
                break
        ...
        if self.is_collided():
            print("COLLIDED!")
            terminal = True
            reward = random.uniform(random.uniform(-10, -5), random.uniform(-4, -1))
            # reward = -3
            self.__init__()
